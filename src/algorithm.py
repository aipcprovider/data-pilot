...
ORM Lite (ormlite)
---------------------------------
This module implements very small ORM over sqlite.
...
class OrmliteResult:
    ...
    def __repr__(self) -> str:
        return f"<OrmliteResult result={self.result!r} meta={self.meta}>"

def core_algorithm(data: Any, **kwargs: Any) -> OrmliteResult:
    """
    Core algorithm stub.
    Parameters
    ----------
    data : Any
        Input structure (very small ORM over sqlite).
    kwargs : Any
    ...
    return OrmliteResult(result={"acc": acc, "trace": trace}, meta=meta)
...
def pretty_print(result: OrmliteResult) -> str:
    """Create a human-readable summary."""
    acc = result.result.get("acc")
    pts = result.meta.get("trace_points")
    return f"ORM Lite: acc={acc:.4f}, trace_points={pts}"
